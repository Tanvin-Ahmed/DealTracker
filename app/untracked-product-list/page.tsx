...
import { getAllProducts, getTotalProductCount } from "@/lib/actions";
...
const UntrackedProductListPage = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const products = await getAllProducts(20, 0);
  ...
  const handleGetNextProducts = async () => {
    setLoading(true);
    const products = await getAllProducts(20, page);
...
export default UntrackedProductListPage;
